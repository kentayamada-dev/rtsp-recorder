...
import { deleteFiles, formatDate, generateCronSchedule, getFilesByExtension, handleError, validatePath } from "@main/utils";
...
export const setupEventHandlers = (sendEvent: SendEvent, mainWindow: BrowserWindow) => {
  const ffmpegExe = config["dev"]
    ? join(app.getAppPath(), `node_modules/ffmpeg-static/${config["files"]["ffmpeg"]}`)
  ...
  registerEventHandlers({
    ...
    "upload:start": (_event, { fps, inputFolder, numberUpload }) => {
      scheduledUploadTask = schedule(generateCronSchedule(numberUpload), async () => {
        try {
          const today = new Date();
          const videoTitle = formatDate(today).second;

          const images = await getFilesByExtension(inputFolder, config["files"]["imageExt"]);
